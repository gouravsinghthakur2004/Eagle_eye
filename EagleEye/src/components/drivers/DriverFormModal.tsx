...
interface DriverFormModalProps {
  ...
  onSave: (savedDriver: DriverNavigatorProfile) => void;
  onClose: () => void;
}
...
export const DriverFormModal: React.FC<DriverFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    setFieldErrors({});
    if (initialValues) {
      setFormData({ ...initialValues, role_type: 'driver' });
    } else {
      setFormData({
        role_type: 'driver',
        full_name: '',
  ...
  const validateCurrentStep = (): boolean => {
    ...
    if (currentStep === 1) {
      const nameVal = validateName(formData.full_name || '', 'Driver Full Legal Name');
      if (!nameVal.isValid) {
        const msg = nameVal.error || 'Driver Full Legal Name is required.';
        errors.push(msg);
    ...
    if (currentStep === 2) {
      ...
      if (isDuplicate) {
        const msg = 'This mobile number is already registered for another Driver.';
        errors.push(msg);
        fErrors.mobile_no = msg;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const finalData: Partial<DriverNavigatorProfile> = { ...formData, role_type: 'driver' };
      ...
      if (res.success && res.data) {
        showSuccess('Driver Saved 🏁', 'Driver profile saved successfully!');
        onSave(res.data);
        onClose();
      } else {
        showError('Save Failed', res.message || 'Could not save driver.');
      }
    } catch (err: any) {
      setSaving(false);
      showError('Error', err.message || 'Driver save failed.');
    }
  };
